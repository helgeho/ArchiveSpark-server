...
object ArchiveSparkServer {
  ...
  def getEnrichFunctions(definitions: String): Seq[EnrichFunc[EnrichRoot, _]] = {
    env.eval(definitions).map(_.asInstanceOf[EnrichFunc[EnrichRoot, _]])
  }
  ...
  def callAction[A <: EnrichRoot : ClassTag](name: String, rdd: => RDD[A], funcs: Seq[EnrichFunc[_ >: A <: EnrichRoot, _]], request: HttpServletRequest, queries: Map[String, String]): Any = {
    actions.asInstanceOf[TypedActions[A]].get(name) match {
      case Some(action) => action(rdd, funcs.map(_.asInstanceOf[EnrichFunc[A, _]]), request, queries)
